...
path_in = "/home/leidinice/Documentos/esi/dados/esi/clim/12WK/"
pathout_fig = "/home/leidinice/Documentos/esi/dados/esi/clim/12WK/"
...
for i, season in enumerate(trim):
    ...
    for ANO in range (2014, 2016 + 1):
        ...
        name = 'esi_12WK_SMN_{0:4d}_SA.nc'.format(ANO)
        data = netCDF4.Dataset(str(path_in) + name)
        aux_in = data.variables['esi'][dic_esi[season], :, :] # Declaring variable
        ...
        aux_in = data.variables['esi'][dic_esi[season], min_lat_index:max_lat_index, min_lon_index:max_lon_index]
        aux_in = np.expand_dims(aux_in, axis=0)
        ...
        cor1 = ('#750000', '#ff0000', '#ff8000', '#fcd17d', '#ffff00', '#ffffff', '#00ffff', '#7dd1fa', '#0080ff',
                '#0000ff', '#000075')  # colors
        lev1 = (-2., -1.6, -1.3, -0.8, -0.5, 0.5, 0.8, 1.3, 1.6, 2.)

        figou1 = 'ESI_Regiao_NEB_12WK_{0}_{1}.png'.format(trim[i], ANO)
        title1 = u'Índice de Estresse Evaporativo \n{0} - {1}'.format(trim[i], ANO)

        pm.plotmap(
        Dmasked[0, :, :], lats, lons, latsouthpoint=y1, latnorthpoint=y2, lonwestpoint=x1, loneastpoint=x2,
