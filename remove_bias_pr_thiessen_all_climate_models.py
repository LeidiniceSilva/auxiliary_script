...
"""
This script remove monthly mean precipitation bias from NMME, ECHAM4.6 and
...
__date__ = '03/30/2017'
__description__='This script remove monthly mean precipitation bias from NMME,' \
...
def arguments():
    ...
    parser.add_argument('--model_name', choices=['echam46', 'rsm2008', 'NMME', 'all'],
...
def define_dates(target_date):
    ...
    end_fcstdate = '{0}{1:02d}{2:02d}'.format(target_efcstdate.year,
                                      target_efcstdate.month, calendar.monthrange
                                      (target_efcstdate.year, target_efcstdate.month)[1])
...
def import_fcstmodel_data(model_name, target_date, basin):
    ...
    if model_name=='echam46' or model_name=='rsm2008':
        ...
        dir_modelname = '{0}{1}/{2}/{3}/{4}/monthly/{5}_thiessen/{6}/'. \
            format(home, localdir, model_name, hind_period, str_mon, var_name,
                   basin_name)

    else:
        hind_period = 'hind8210'
        dir_modelname = '{0}{1}/nmme/{2}/{3}/{4}/monthly/{5}_thiessen/{6}/'. \
            format(home, localdir, model_name, hind_period, str_mon, var_name,
                   basin_name)

    file_modelname = "{0}_monthly_{1}_{2}_fcst_{3}_{4}_{5}_thiessen_{6}.nc". \
...
def import_hindobs_data(model_name, target_date, basin, idx):
    ...
    file_name = '{0}/io/inmet_ana_chirps/calibration/{1}/' \
                'pr_thiessen/{2}/{3}_{1}_{4}_obs_{5}_thiessen_{6}.nc' \
              .format(home, time_freq, basin_name, var_name, data_base,
                hind_obs_period, basin_full_name)

    try:
        inc = Dataset(file_name)
        time_var = inc.variables['time']
        iidx = date2index(datetime(ihind, 01, 01), time_var)
        fidx = date2index(datetime(fhind, 12, 31), time_var)
        var = inc.variables[var_name][iidx:fidx + 1]
        return var[fcstdate.month-1:len(var):12]
    except:
        print 'Observational Calibration File: {0} is not available'. \
            format(os.path.join(file_name))
        exit()
...
def gamma_correction(hind, clim_obs, fcst):
    ...
    # Zero probability in observational serie!
        ...
        for basin in basin_list:
            ...
            if flag:
                ...
                if modname == 'echam46' or modname == 'rsm2008':
                    ...
                    hindname = 'hind8110'
                    path_out = '{0}{1}/{2}/{3}/{4}/monthly/{5}_thiessen_cor/{6}/'.format(home, 
                           localdir, modname, hindname, str_mon, var_name, basin_name)
                else:
                    hind = np.full((3,29), np.nan)
                    obs = np.full((3,29), np.nan)
                    hindname = 'hind8210'
                    path_out = '{0}{1}/nmme/{2}/{3}/{4}/monthly/{5}_thiessen_cor/{6}/'.format(home,
                            localdir, modname, hindname, str_mon, var_name, basin_name)
                
                if not os.path.exists(path_out):
                    os.makedirs(path_out)
                    
                fcst = np.nansum(fcst_list) # Accumulating the trimester
                
                # Import hindcast and observational data.
                for idx in range(0, 3):
                    hind[idx, :] = import_hindmodel_data(modname, target_date,
                                                        basin, idx)
                    obs[idx, :] = import_hindobs_data(modname, target_date,
                                                        basin, idx)
                ...
                # Saving data
                write_thiessen(corr, start_fcstdate, end_fcstdate, time_freq,
                   var_name, '{0}_{1}'.format(modname, hindname), 'fcst',
                   '{0}_cor'.format(basin_full_name), init_date=start_rundate,
