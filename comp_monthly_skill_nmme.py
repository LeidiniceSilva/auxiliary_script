...
macros_total = ['amazonas', 'atlantico_leste', 'atlantico_sudeste', 'atlantico_sul', 'doce', 'grande', 'iguacu', 'jacui',
          'paraguai', 'paraiba_do_sul', 'parana', 'paranaiba', 'paranapanema', 'parnaiba', 'sao_francisco', 'tiete',
          'tocantins', 'uruguai']
    ...
    for model in models:
        ...
        for basin in bas_new:
           ...
           # open netcdf
            ...
            stc1.append(st1[9::12])
            stc2.append(st1[10::12])
            stc3.append(st1[11::12])
            ...
            link2 = home + "/io/nmme/{0}/{1}/{2}/{3}/{4}_thiessen/{5}".format(model, period, mon, scale, param,
                                                                              macro_name)
            for year in range(1982, 2011):
                arq2 = "{0}/{1}_{2}_{3}_{4}_fcst_{5}0901_{5}1001_{5}1231_thiessen_{6}.nc".format(link2, param, scale,
                                                                                                 model, period, year,
                                                                                                 basin_fullname)
        ...
        for i in range(len(bas_new)):
            ...
            fig.suptitle(u'Índices: CORRELAÇÃO, MBE e RMSE \n Modelo: {0} / Previsão: {1} - {2} \n Bacia: {3} - Usinas:'
                         u' Incrementais'.format(model.upper(), month.capitalize(), period.upper(),
        ...
        path_out = ('/home/leidinice/documentos/results/check_skill_nmme/{0}/{1}/{2}/{3}/{4}_thiessen/{5}/'
                    'incrementais/'.format(model, period, mon, scale, param, macro_name))

        if not os.path.exists(path_out):
            create_path(path_out)

        plt.savefig(os.path.join(path_out, '{0}_incrementais_indices_CORREL_MBE_RMSE_{1}_thiessen_{2}_{3}_{4}'
