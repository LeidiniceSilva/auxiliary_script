...
def spi(data, scale, nseas):
    ...
    for i in range(scale):
        # A1=np.append(A1,data[i:len(data)-scale+i+1])
    ...
    if scale > 1:
        index = np.arange(nseas*erase_yr-scale+1)
    ...
    for i in range(nseas):
        ...
        Xn_nozero = Xn[np.nonzero(Xn)]
        q = len(zeroa)/np.float(len(Xn))
