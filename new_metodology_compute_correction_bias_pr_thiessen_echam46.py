...
if __name__ == '__main__':
    ...
    for basin in basins:
        ...
        plt.title(u'Pr_Thiessen - viés corrigido do acumulado mensal- Ago (S-O-N)\n bacia {0}'.format(basin_fullname))
        plt.ylim(0, 700)
        plt.ylabel(u'mm')
        plt.xlabel(u'anos')
        legenda = ('OBS', 'ECHAM_bru', 'ECHAM46_corri')
        plt.legend(legenda, frameon=False)
        path_out1 = ('{0}/check_echam46_obs_basins/pr_thiessen/pr_thiessen_monthly_echam46_corrected_operation/acc_gamma/figures/ago/{1}/'.format(hidropy_path, basin_dict(basin)[1]))
        path_out2 = ('{0}/check_echam46_obs_basins/pr_thiessen/pr_thiessen_monthly_echam46_corrected_operation/acc_gamma/ago/{1}/'.format(hidropy_path, basin_dict(basin)[1]))

        if not os.path.exists(path_out1):
            create_path(path_out1)

        plt.savefig(os.path.join(path_out1, 'pr_thiessen_acc_obs_echam46_corrigido_{0}.png'.format(basin_fullname)))
        plt.close('all')
        plt.cla()
        ...
        for k, yea in enumerate(range(2011, 2017)):
            ...
            name_nc = write_thiessen(aux, new_y, end_y, 'monthly', 'pr', 'echam46_hind8110', 'fcst', 'corrected_{0}'.format(basin_fullname),
