    ...
    for macro in macros_inc:
        ...
        for basin in bas_new:
            ...
            for year2 in range(2011, 2017):
                arq4 = "{0}/{1}_{2}_{3}_{4}_fcst_{5}0101_{5}0201_{5}0430_thiessen_{6}.nc".format(link4, param,  scale,
                                                                                             model, period, year2,
                                                                                             basin_fullname)
        ...
        for i in range(len(bas_new)):
            ...
            fig.suptitle(u'Correções: GAMMA, GUMBEL e GAMMA DESAG. \n Modelo: {0} / Previsão: {1} - {2} \n Bacia:'
                         u' {3} - Usinas: Incrementais'.format(model.upper(), month.capitalize(), period.upper(),
        ...
        path_out = ('/home/leidinice/documentos/results/check_skill_echam46/{0}/{1}/{2}/{3}/{4}_thiessen/{5}/'
                    'incrementais/'.format(model, period, month, scale, param, macro_name))
