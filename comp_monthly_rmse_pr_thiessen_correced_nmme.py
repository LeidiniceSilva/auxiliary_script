...
for macro in macros_total:
    ...
    for model in models:
        ...
        for basin in bas_new:
            ...
            stc_obs_acc.append(st1[1::12] + st1[2::12] + st1[3::12])
            ...
            observ1 = np.full(72, np.nan)
            observ1[0:48] = st3
            observ1[48:72] = st5

            observ2 = np.full(62, np.nan)
            observ2[0:36] = st4
            observ2[36:62] = st6

            sto1.append(observ1[1::12])
            sto2.append(observ1[2:12])
            sto3.append(observ1[3::12])
            ...
            link3 = home + "/io/nmme/{0}/{1}/{2}/{3}/{4}_smap/{5}".format(model, period, mon, scale, param, macro_name)
            for year1 in range(1982, 2011):
                ...
                arq3 = "{0}/{1}_{2}_{3}_{4}_fcst_{5}0101_{5}0215_{5}0415_smap_{6}.nc".format(link1, param, scale, model,
                                                                                             period, year1, basin_fullname)
            ...
            link4 = home + "/io/nmme/{0}/{1}/{2}/{3}/{4}_smap/{5}".format(model, period, mon, scale, param, macro_name)
            sto1_new = []
            sto2_new = []
            sto3_new = []
            ...
            for ia, year2 in enumerate(range(2011, 2017)):
                ...
                new_year_ = start_date_ + relativedelta(months=1)
                new_year_y_ = str(new_year_)[0:4] + str(new_year_)[5:7] + str(new_year_)[8:10]

                arq4 = "{0}/{1}_{2}_{3}_{4}_fcst_{5}0101_{5}0215_{5}0415_smap_{6}.nc".format(link1, param, scale, model,
                                                                                             period, year2, basin_fullname)
        ...
        for i in range(len(bas_new)):
            ...
            fig.suptitle(u'Correções: GAMMA, GUMBEL e GAMMA DESAG. \n Modelo: {0} / Previsão: {1} - 2011_2016 \n Bacia:'
                         u' {2} - Usinas: Totais'.format(model.upper(), month.capitalize(), macro_name.capitalize()),
        ...
        path_out = ('/home/leidinice/documentos/results/nmme/correc_fcst1116/{0}/{1}/{2}/{3}/{4}_'
                    'smap/{5}/totais/'.format(model, period, mon, scale, param, macro_name))

        if not os.path.exists(path_out):
            create_path(path_out)

        plt.savefig(os.path.join(path_out, '{0}_totais_indices_RMSE_{1}_smap_{2}_{3}_'
