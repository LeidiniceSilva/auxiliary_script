...
def caso_shape(data, lat, lon, shp):
    ...
    plt.plot(xx, yy, color='k',  linewidth=1.5)
    plt.show()
...
path_in = "/home/leidinice/Documentos/esi/results/data/spi/" # Directory entry of  data

trim = ['NDJ', 'DJF', 'JFM', 'FMA', 'MAM', 'AMJ', 'MJJ', 'JJA', 'JAS', 'ASO', 'SON', 'OND']
dic_esi = {'NDJ': 0, 'DJF': 1, 'JFM': 2, 'FMA': 3, 'MAM': 4, 'AMJ': 5, 'MJJ': 6, 'JJA': 7, 'JAS': 8, 'ASO': 9,
           'SON': 10, 'OND': 11}
...
for ANO in range(2009, 2012 + 1):
    ...
    for season in trim:
        ...
        lon = data.variables['lon'][:] # Declaring longitude
        ...
        figou1 = 'plot_spi_3_{0}_{1}.png'.format(season, ANO)
        title1 = u'a. SPI 3 - Bacia do Rio São Francisco - {0} - {1}'.format(season, ANO)
